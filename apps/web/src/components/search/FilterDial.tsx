...
export function FilterDial({ value, onChange, onOpenChange }: Props) {
  ...
  const sections = useMemo(() => {
    ...
    return [
      ...
      {
        title: t("filters.composition"),
        key: "composition" as const,
        items: filt(taxonomy.compositions || []),
      },
      {
        title: t("filters.techniques"),
        key: "technique" as const,
        items: filt(taxonomy.techniques || []),
      },
      { title: t("filters.era"), key: "era" as const, items: filt(taxonomy.eras || []) },
      { title: t("filters.origin"), key: "origin" as const, items: filt(taxonomy.origins || []) },
      { title: t("filters.ism"), key: "ism" as const, items: filt(taxonomy.isms || []) },
      {
        title: t("filters.style"),
        key: "visualStyle" as const,
        items: filt(taxonomy.visual_styles || []),
      },
      { title: t("filters.theme"), key: "theme" as const, items: filt(taxonomy.themes || []) },
      { title: t("filters.genre"), key: "genre" as const, items: filt(taxonomy.genres || []) },
      { title: t("filters.shape"), key: "shape" as const, items: filt(taxonomy.shapes || []) },
      { title: t("filters.emotion"), key: "emotion" as const, items: filt(taxonomy.emotions || []) },
      {
          ...
          <div className="max-h-[60vh] space-y-4 overflow-y-auto p-3">
            <div>
              ...
              <input
                ...
                placeholder={t("filters.moodPlaceholder")}
            ...
            <div>
              ...
              <input
                ...
                placeholder={t("filters.directorPlaceholder")}
                className="w-full max-w-sm rounded border border-cinema-border bg-cinema-black px-2 py-1.5 text-xs text-white outline-none focus:border-cinema-cyan"
              />
            </div>
            {sections.map((sec) => (
